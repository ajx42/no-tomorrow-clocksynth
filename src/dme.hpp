...
inline DMENode merge( const DMENode& lhs, const DMENode& rhs, wire wr ) {
  ...
  double eaDbl = (double)((del2 - del1) + (double)(d*d*wr.resistance*wr.cap)/2 + d*wr.resistance*c2) / (wr.resistance*(c1 + c2 + d));
...
inline EmbeddingManager::EmbeddingManager( inparams inp, clksyn::TopologyResult res )
...
inline void EmbeddingManager::dfs( int32_t nodeIdx, int32_t parentIdx ) {
  ...
  if ( kidOne == -1 ) {
    ...
    nodes_[nodeIdx] = DMENode {
      .Core = DMECore{ .Kind = DMECore::POINT, .Loc = pt_t{.x = topoNode.x, .y = topoNode.y} },
      .Delay = 0,
